...
class GPTNeoXAttention(nn.Module):
    ...
    def forward(
        ...
        passed_key_value=None,
    ):
        has_layer_past = layer_past is not None

        # Compute QKV
        # Attention heads [batch, seq_len, hidden_size]
        #   --> [batch, seq_len, (np * head_size)]
        ...
        query = q.view(*new_q_shape).permute(0, 2, 1, 3)

        if self.has_kv:
            k = self.key(hidden_states)
            v = self.value(hidden_states)

            # [batch, seq_len, (num_kv_heads * head_size)]
            #   --> [batch, seq_len, num_kv_heads, head_size]
            new_kv_shape = k.size()[:-1] + (self.num_key_value_heads, self.head_size)
            #  --> [batch, num_kv_heads, seq_len, head_size]
            key = k.view(*new_kv_shape).permute(0, 2, 1, 3)
            value = v.view(*new_kv_shape).permute(0, 2, 1, 3)
            passed_key_value = (key, value)
        else:
            key = passed_key_value[0]
            value = passed_key_value[1]
        ...
        if self.has_kv:
            ...
            query, _ = apply_rotary_pos_emb(query_rot, query_rot, cos, sin, offset=offset)
        ...
        return outputs, passed_key_value
...
class GPTNeoXLayer(nn.Module):
    def __init__(self, config, has_kv=True):
        ...
        self.attention = GPTNeoXAttention(config, has_kv=has_kv)
    ...
    def forward(
        ...
        attention_layer_outputs, passed_key_value = self.attention(
            ...
            passed_key_value=passed_key_value,
        )
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([GPTNeoXLayer(config, (l in self.key_value_layers)) for l in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        if past_key_values is None:
            past_key_values = tuple([None] * self.config.num_key_value_layers)
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                passed_key_value = outputs[-1]
            else:
                if i in self.key_value_layers:
                    layer_past = past_key_values[self.key_value_layers.index(i)]
                outputs = layer(
                    hidden_states,
                    attention_mask=attention_mask,
