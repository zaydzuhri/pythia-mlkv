...
class GPTNeoXAttention(nn.Module):
    ...
    def forward(
        ...
        if has_layer_past:
            ...
            # seq_len -= 1 if self.has_kv else 0
        cos, sin = self.rotary_emb(query, seq_len=seq_len)
        if self.has_kv:
            query, key = apply_rotary_pos_emb(query_rot, key_rot, cos, sin, position_ids)
        else:
            query, _ = apply_rotary_pos_emb(query_rot, query_rot, cos, sin, position_ids)
        query = torch.cat((query, query_pass), dim=-1)
        if self.has_kv:
            key = torch.cat((key, key_pass), dim=-1)

        # Cache QKV values
        if has_layer_past and self.has_kv:
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if use_cache is True:
                last_present = outputs[1]
                if i in self.key_value_layers:
                    presents = presents + (last_present,)
            if output_attentions:
